...
class GraphBuilder:
    ...
    def build_lidar_ros_graph(self, full_cfg: dict, graph_path: str = "/LidarGraph"):
        ...
        if lidar.get("create_3d", False):
            ...
            # RenderProduct의 역할
                # 매 프레임마다 RTX 레이캐스트 수행 → 버퍼 생성
                # LiDAR 헬퍼가 RenderProductPath를 읽어 ROS 메시지로 직렬화
                # UI뷰포트와 무관 → 헤드리스에서도 동일하게 동작

            setvals += [ # 내부 레이캐스트 샘플링 해상도 역할 (예: 640 * 480) => 샘플링 밀도(출력 품질)에 직접 영향
                ("RP_3D.inputs:enabled",     True),
                ("RP_3D.inputs:cameraPrim",  Sdf.Path(lidar["prim_3d"])), # 센서 속성 (실제 각도/빔 수/회전 속도): LiDAR 프림의 속성에서 결정
                ("RP_3D.inputs:width",       640),
                ("RP_3D.inputs:height",      480),
                # 샘플링 결과 운반: 속성에 의해 결정된 샘플링 결과를 RenderProduct가 운반
                # 고해상도/비균일 패턴이 필요하면 →  LiDAR 프림 + RP 해상도 모두 조정

                ("Lidar3D.inputs:enabled",         True),
                ("Lidar3D.inputs:type",            "point_cloud"),
                ("Lidar3D.inputs:topicName",       topics["point_cloud"]), # 토픽: /point_cloud : sensor_msgs/PointCloud2
                ("Lidar3D.inputs:frameId",         frames["base_scan_3d"]), # frameId: 3D → frames["base_scan_3D"]
